...
export default function NewJournalPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const MOOD_OPTIONS = [
    { value: 'great', label: j.moodGreat },
    { value: 'good',  label: j.moodGood },
    { value: 'okay',  label: j.moodOkay },
    { value: 'bad',   label: j.moodBad },
  ] as const
  ...
  async function submit() {
    start(async () => {
      ...
      if (!res.ok) { toast.error(data.error ?? j.saveError); return }

      if (data.exifStripped) {
        toast.success(j.savedGps(data.xpEarned))
      } else {
        toast.success(j.saved(data.xpEarned))
      }
  ...
  return (
    <div style={{ maxWidth: '560px', padding: '16px 16px 60px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        {/* Mood */}
        <div>
          <label style={labelStyle}>{j.moodLabel}</label>
        ...
        {/* Photo */}
        <div>
          <label style={labelStyle}>{j.photoLabel}</label>
          {photoPreview ? (
        ...
        {/* Measurements */}
        <div>
          <label style={labelStyle}>{j.measureLabel}</label>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
            {[
              { label: j.temperature, val: temperature, set: setTemperature, placeholder: '24' },
              { label: j.humidity,    val: humidity,    set: setHumidity,    placeholder: '60' },
              { label: j.ph,          val: ph,          set: setPh,          placeholder: '6.5' },
              { label: j.ec,          val: ec,          set: setEc,          placeholder: '1.4' },
              { label: j.water,       val: waterAmount, set: setWaterAmount, placeholder: '500' },
        ...
        {/* Notes */}
        <div>
          <label style={labelStyle}>{j.notesLabel}</label>
          <textarea
            value={notes}
          ...
          <div style={{ fontFamily: 'var(--font-dm-mono)', fontSize: '8px', color: '#4a6066', marginTop: '4px', textAlign: 'right' }}>
            {notes.length}/500 {notes.length > 50 ? `· ${j.xpBonus}` : ''}
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ fontFamily: 'var(--font-dm-mono)', fontSize: '9px', color: '#f0a830' }}>
            {j.xpPreview(15 + (photo ? 20 : 0) + (temperature && humidity ? 10 : 0) + (ph ? 10 : 0) + (notes.length > 50 ? 15 : 0))}
          </div>
        </div>
