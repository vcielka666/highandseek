...
export async function GET() {
  ...
  const result = strains.map(strain => {
    ...
    if (raw) {
      ...
      userState = {
        ...
        needs:     { hydration: Math.round(needs.hydration), nutrients: Math.round(needs.nutrients), energy: Math.round(needs.energy), happiness: Math.round(needs.happiness) },
    ...
    return {
      ...
      personality:    { archetype: strain.personality.archetype, catchphrase: strain.personality.catchphrase, tone: strain.personality.tone },
  ...
  return NextResponse.json({ strains: result })
}
